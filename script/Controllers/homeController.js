'use strict';
app.controller('homeController', ['$scope', 'matrixService', '$timeout', 'connectionService', function ($scope, matrixService,  $timeout, connectionService) {
    ...
    $scope.showLines = function (truckNum){
        ...
        var coordinates = [];
        coordinates.push( new google.maps.LatLng($scope.factoryLocation.lat, $scope.factoryLocation.lng));
    ...
    $scope.getMatrix = function (iter_){
        ...
        var fact_ = new google.maps.LatLng($scope.factoryLocation.lat, $scope.factoryLocation.lng);
        addArr_.push(fact_);
    ...
    $scope.initialize = function () {
            ...
            var myLatlng = new google.maps.LatLng(59.326142, 17.9875454);
    ...
    var saveToQueues = function (lat_, long_){
        ...
        if(!!!$scope.factoryLocation)
        {
            $scope.$apply(function () { 
                 $scope.factoryLocation = {lat:lat_, 
                                            lng:long_, 
                                            start: $scope.workTime.start, 
                                            end: $scope.workTime.end};
             });
